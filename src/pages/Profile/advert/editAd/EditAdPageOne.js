...
import {
  ...
  editSubCategoryId,
  editAdvertItem,
} from "../../../../features/userAdvertSlice";
import { AdContainer } from "../../../PlaceAd/PlaceAd.styled"; 
...
export default function EditAdPageOne({ onNextPage }) {
  ...
  const cloudinaryUpload = async (photo) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        dispatch(appendAdImages(response.data?.url));
        // console.log(response.data);
      }
    } catch (err) {
      setFetchingImage(false);
  ...
  const handleDrop = (e, index) => {
    ...
    dispatch(updateAdImages(updatedURLs));
  ...
  const handleInputChange = (e) => {
    dispatch(editAdvertItem({ [e.target.name]: e.target.value }));
  };
  ...
  const handleSelectCategories = (e) => {
    ...
    dispatch(editCategoryId(selectedCategory.id));
    setSubCategories(
        ...
        <div className="inputContainer">
          ...
          <select
            ...
            onChange={(e) => {
              ...
              dispatch(editSubCategoryId(selectedSubCategory?.id));
            }}
            required
            name="subCategory"
            id="sub-Category"
